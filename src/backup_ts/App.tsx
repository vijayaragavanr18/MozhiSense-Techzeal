...
import { Screen, Question, WordDetail } from './types';
import { MOCK_USER, WORD_MASTERY, RECENT_ACTIVITY, FEATURED_LESSON, QUESTIONS, WORD_DETAILS } from './data';

// --- Components ---

const TopBar = ({ xp }: { xp: number }) => (
...
const PlayScreen = ({ addXp, streak, setStreak }: { 
  ...
  const [subTab, setSubTab] = useState<'challenge' | 'practice' | 'review'>('challenge');
  ...
  const handleDragEnd = (_event: any, info: any, idx: number) => {
...
const ProfileScreen = ({ theme, setTheme, xp, streak }: { 
  theme: 'light' | 'dark' | 'white-green', 
  setTheme: (t: 'light' | 'dark' | 'white-green') => void,
  xp: number,
  streak: number
}) => (
  <div className="space-y-12 pb-20">
...
export default function App() {
  ...
  const [theme, setTheme] = useState<'light' | 'dark' | 'white-green'>(() => {
    const saved = localStorage.getItem('mozhi-theme');
    return (saved as 'light' | 'dark' | 'white-green') || 'dark';
  });
  ...
  const [selectedWord, setSelectedWord] = useState<string>('படி');
